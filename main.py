...
# customizable:
client_dir = "E:\Riot Games\League of Legends/LeagueClient.exe"
    ...
    while tries > 0:
        if process_exists(client_process) and \
            ...
            if pyautogui.locateOnScreen(image=images.get("nexus"), confidence=0.8, grayscale=True) is not None:
                state = "game"
                return
            missions = pyautogui.locateOnScreen(image=images.get("play"), confidence=0.8, grayscale=True)
            if missions is not None:
                pyautogui.moveTo(x=missions[0], y=missions[1] + 300)
                click_button(image=images.get("play"))
                time.sleep(1)
                click_button(image=images.get("play"))
            else:
                click_button(image=images.get("play"))
